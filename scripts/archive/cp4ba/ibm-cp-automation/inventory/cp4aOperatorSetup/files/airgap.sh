...
validate_registry_service_init_arguments() {
    if [[ "$REGISTRY_HOST" == *':'* ]]; then
        echo "[ERROR] The --registry parameter shouldn't contain port in the initialization stage. Port should be specified while starting the registry."
        exit 1
    fi
...
do_registry_secret_create() {
    ...
    echo "[INFO] Creating registry authentication secret for ${AUTH_REGISTRY_SERVER}"
...
validate_registry_secret_create_arguments() {
    if [[ "$AUTH_REGISTRY_SERVER" == *'://'* ]]; then
        echo "[ERROR] The registry server address should match the specific definition of an IP or FQDN which do not include a protocol."
        exit 1
    fi
...
do_image_mirror_single_image() {
    ...
    adjust_dry_run_flag 'image mirror'
    oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${IMAGE}\" \"${TARGET_REGISTRY}/${image_identifier}\" --filter-by-os '.*' --insecure ${DRY_RUN} ${OC_DEBUG_LOGGING}" 
    echo "${oc_cmd}"
    eval ${oc_cmd}
...
do_skopeo_copy_single_image() {
    ...
    oc_cmd="skopeo copy --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false \"docker://${IMAGE}\" \"docker://${TARGET_REGISTRY}/${image_identifier}\" ${SKOPEO_DEBUG_LOGGING}"
    echo "${oc_cmd}"
    eval ${oc_cmd}
...
do_image_mirror_case_images() {
    ...
    for map_file in ${map_files}; do
        ...
        adjust_dry_run_flag 'image mirror'
        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" -f \"${map_file}\" --filter-by-os '.*' --insecure ${DRY_RUN} ${OC_DEBUG_LOGGING}"
        echo "${oc_cmd}"
        eval ${oc_cmd}
    ...
    if [[ ${images_count} == 0 ]]; then
      for mirrored_csv in ${MIRRORED_IMAGE_CSV_CONTEXT[@]}; do
        if [[ ${mirrored_csv} == *"-delta-images-"* ]]; then
          cleanup_mirrored_csv_files_from_current_run
          echo "[INFO] Found 0 images that had not already been mirrored. No additional work to do. Use the --skip-delta argument to force images to be mirrored."
          exit 0
        fi
      done
      cleanup_mirrored_csv_files_from_current_run
...
do_skopeo_copy_case_images() {
    ...
    while read in; do
        oc_cmd="skopeo copy --all --authfile \"${AUTH_DATA_PATH}/auth.json\" --dest-tls-verify=false --src-tls-verify=false docker://${in} ${SKOPEO_DEBUG_LOGGING}" ;
        echo "${oc_cmd}"
        eval ${oc_cmd}
            ...
            if [[ "$(is_valid_olm_catalog_tag ${tag})" == "true" ]]; then
                ...
                if [[ ! -z "${all_tags}" ]]; then
                    ...
                    if [[ ${last_tag} =~ ^${tag_significane} ]]; then
                        ...
                        adjust_dry_run_flag 'image mirror'
                        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${image}@${sha}\" \"${image}:${latest_tag}\" --filter-by-os '.*' --insecure ${DRY_RUN} ${OC_DEBUG_LOGGING}"
                        echo "${oc_cmd}"
                        eval ${oc_cmd}
...
process_case_csv_file() {
    ...
    if [[ ! -z ${IMAGE_MIRROR_ACTION_CALLED} ]] && [[ $(wc -l < "${tmp_csv_file}") -gt 0 ]]; then 
...
do_cluster_update_pull_secret() {
    ...
    if [[ "${current_pull_secret}" =~ "\"${TARGET_REGISTRY}\":" ]]; then
        echo "[INFO] Target registry authentication secret already present in the cluster. Overwriting the secret with a new one"
