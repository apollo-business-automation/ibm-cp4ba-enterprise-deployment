...
OPTIONS:
 ...
 ARGS per Action:
    ...
    configure-cluster-airgap
      ...
      --nsPrefix               : Append a prefix to the namespace of the mirror image
    ...
    mirror-images
      ...
      --filter                 : list of filter(s) to apply to mirror (comma separated)
    ...
    install
      ...
      --filter                 : list of filter(s) to apply to install (comma separated)
    ...
    install-catalog:
      ...
      --filter                 : list of filter(s) to apply to catalog (comma separated)
    ...
    post-install               : entry point for any product post-install actions
    
    validate-install           : entry point for any product validate-install actions
"
...
validate_mirror_images_args() {
    # Verify arguments required to mirror images were provided
    local foundError=0
    [[ -z "${registry}" ]] && {
        foundError=1
        err "'--registry' must be specified with the '--args' parameter"
...
translate_filters() {
   # Translate '--filter' parameter into appropriate parameters for airgap.sh (ie. --groups, --arch)
   [[ -n "${image_filters}" ]] && {
     _IFS=$IFS
     IFS=","
     read -ra filters <<<"${image_filters}"
     IFS="$_IFS"
     for i in "${filters[@]}"; do
       case "$i" in
        amd64 | ppc64le | s390x | arm64)
            [[ -n "${image_arch}" ]] && { image_arch="${image_arch},${i}" ; } # Append arch if already defined
            : "${image_arch:=$i}" # Set first arch parameter 
            ;;
        *)
            [[ -n "${image_groups}" ]] && { image_groups="${image_groups},${i}" ; } # Append groups if already defined
            : "${image_groups:=$i}" # Set first group parameter 
            ;;
        esac
     done
   }
